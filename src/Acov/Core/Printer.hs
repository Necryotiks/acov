...
import Acov.Frontend.SymbolTable (Symbol, SymbolTable, stNameAt)
import Acov.Frontend.VInt (printVInt)
...
showExprRec syms that rexpr =
  let (this, str) = showExpression' syms (rangedData rexpr)
   in parenthesize this str that
...
showExpression' syms (E.ExprSel rsym ra rb) =
  ( 100,
    symName syms (rangedData rsym) ++ "["
      ++ se ra
      ++ (case rb of Nothing -> "]"; Just rb' -> ":" ++ se rb' ++ "]")
  )
  where
    se = showExprRec syms 1
showExpression' syms (E.ExprConcat re0 res) =
  ( 100,
    "{" ++ se re0 ++ concatMap (\re -> ", " ++ se re) res ++ "}"
  )
  where
    se = showExprRec syms 14
