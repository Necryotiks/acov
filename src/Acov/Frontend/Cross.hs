...
import Acov.Frontend.Count (Count, mkCount)
...
hitsCross (Cross wrls) n =
  let (n', good) = foldr f (n, True) wrls
   in assert (n' == 0) good
  where
    f (w, rl) (n', good) =
      (shiftR n' w, good && rlMember (bitmask w .&. n') rl)
    bitmask w = shiftL (1 :: Integer) w - 1
...
crossCount vals cross = mkCount (Foldable.foldr f 0 vals) (crossSize cross)
  where
    f val k = if hitsCross cross val then 1 + k else k
...
crossList (Cross wrls) =
  assert (not $ null wrls) $
    let lst = rlToList $ snd $ head wrls
     in if null $ tail wrls
          then map (: []) lst
          else
            concatMap (\vals -> map (: vals) lst) $
...
crossMisses n hits cross = f n (crossList cross)
  where
    ...
    f 0 _ = []
    f k (v : vs) =
      if Set.member (glue widths v) hits
        then f k vs
        else v : f (k - 1) vs
