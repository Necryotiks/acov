...
import Acov.Frontend.Ranged (Ranged (..), rangedData, rangedRange)
...
wrapRecord guards scopes stmt = Group guards scopes record
  where
    recExpr_ = P.recExpr stmt
    recSym_ = P.recSym stmt
    record = case P.recCov stmt of
      Nothing ->
        Left [Record recExpr_ recSym_ Nothing]
      Just (P.CoverList vals) ->
        Left [Record recExpr_ recSym_ (Just vals)]
      Just P.CoverBits ->
        Right $ BitsRecord recExpr_ recSym_
...
readStmt guards scopes rstmt = f (rangedData rstmt)
  where
    ...
    f (P.Group (P.Block () stmts)) =
      do
        body <- mapEO groupReadStmt stmts
...
groupReadStmt rstmt = f (rangedData rstmt)
  where
    ...
    g (P.RecordStmt _ _ (Just P.CoverBits)) =
