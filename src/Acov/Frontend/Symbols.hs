...
import qualified Acov.Frontend.Grouping as G
...
import Acov.Frontend.VInt (VInt)
...
readPorts ports = stbToSymbolTable <$> foldEO takePort stbEmpty ports
  where
    takePort bld rp =
      let P.Port sym slice = rangedData rp
       in stbAdd "port" bld (rangedRange rp) (copyRange rp sym) slice
...
readReplicate ::
  ...
  Ranged P.Expression ->
  ErrorsOr Expression
readReplicate ps count val = liftA2 ExprReplicate (read_ count) (read_ val)
  where
    read_ = readExpression ps
...
readBinOp ::
  ...
  Ranged P.Expression ->
  ErrorsOr Expression
readBinOp ps binop e0 e1 = liftA2 (ExprBinOp binop) (read_ e0) (read_ e1)
  where
    read_ = readExpression ps
...
readCond ::
  ...
  Ranged P.Expression ->
  ErrorsOr Expression
readCond ps e0 e1 e2 = liftA3 ExprCond (read_ e0) (read_ e1) (read_ e2)
  where
    read_ = readExpression ps
...
getRecName expr Nothing =
  case rangedData expr of
    ...
    _otherwise -> bad1 $ copyRange expr "Cannot guess a name for recorded expression."
...
takeRecord ps (stb, recs) (G.Record expr as cover) =
  do
    (expr', psym) <- liftA2 (,) (readExpression ps expr) (getRecName expr as)
