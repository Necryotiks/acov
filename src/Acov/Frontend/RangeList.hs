...
module Acov.Frontend.RangeList
  ( RangeList,
    rlLength,
    rlIntervals,
    rlToList,
...
import Acov.Frontend.IvlList
    ( IvlList, ivlListIntervals, ivlListLen )
...
flattenUpd (pairs, Just (low, high), size) low' high' =
  assert (low < low') $
    assert (low' <= high') $
      if low' <= high + 1
        then (pairs, Just (low, max high high'), size)
