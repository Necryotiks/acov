module RangeListSpec (rangeListSpec) where
...
genPair =
  sized
    ( \n -> do
        lo <- oneof [arbitrary, choose (-100, 100)]
        hi <- frequency [(1, min (lo - 1) <$> arbitrary), (10, choose (lo, lo + shiftL lo n))]
        return (lo, hi)
    )
...
instance Arbitrary RLTest where
  arbitrary = RLTest <$> listOf (scale (`quot` 10) genPair)
...
prop_RLMatchesERL (RLTest pairs) =
  ...
  where
    erl = foldr erlAdd erlEmpty pairs
    rl = foldr rlAdd rlEmpty pairs

rangeListSpec :: IO ()
rangeListSpec = hspec $ do
  describe "RangeList" $ do
    it "matches reference implemention" $ property prop_RLMatchesERL
